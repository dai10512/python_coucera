...
def to_mod_list(employee_list):
   ...
   name_department_list = list(map(mod, employee_list))
   return name_department_list
...
def generate_usernames(mod_list):
   ...
   result = [mod.replace(" ","_") for mod in mod_list]
   return result
   raise NotImplementedError()
...
def map_id_to_initial(employee_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   def getInitial(employee):
      return {employee['id']:employee['name'][0]}
   firstIniialList = {employee['name'][0]:employee['id'] for employee in employee_list}
   
   print(firstIniialList)
   # return result 
   return firstIniialList
   raise NotImplementedError()
